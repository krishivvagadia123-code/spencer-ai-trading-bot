...
def _without_frontmatter_updated(text: str) -> str:
    ...
    stable_frontmatter = [
        line for line in frontmatter if not line.strip().startswith("updated:")
    ]
    return "---\n" + "\n".join(stable_frontmatter) + normalized[end:]


def _same_except_frontmatter_updated(existing: str, new_text: str) -> bool:
    return _without_frontmatter_updated(existing) == _without_frontmatter_updated(new_text)


def _write_note_if_changed(path: Path, text: str) -> bool:
    if path.exists():
        try:
            existing = path.read_text(encoding="utf-8")
        except OSError:
            existing = None
        if existing is not None and _same_except_frontmatter_updated(existing, text):
            return False
...
def _note(
    ...
    *,
    ...
    return _write_note_if_changed(brain / f"{name}.md", text)
...
> Generated by the [[Brain Exporter]]; the latest generated timestamp lives in frontmatter.""", stamp)
